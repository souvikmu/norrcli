...
import { logRandomResponse, logCategoryResponse, logArrayResponse, logAvailableCategories, } from "./helpers.js";
export default async function cliPalette() {
    ...
    cli.command("category")
        ...
        try {
            const categories = queryParser(options.category);
    ...
    cli.command("query")
        ...
        .action(async (options) => {
        try {
            const queries = queryParser(options.queries);
